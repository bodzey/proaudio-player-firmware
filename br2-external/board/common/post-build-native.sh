...
# Alert credentials are mutable appliance state. Hardware profiles may provide
# a persistent data mount, but the immutable factory image must never embed a
# real token under /etc.
rm -f "$TARGET_DIR/etc/proaudio-player-alert/alerts-token"

# Embed source/build identity into every native image, independent of the
# target board. The metadata records firmware, native and WebUI revisions and
# marks non-clean source trees.
